...
class AudiosetDataset(Dataset):
    ...
    def pro_data(self, data_json):
        for i in range(len(data_json)):
            try:
                data_json[i] = [data_json[i]['wav'], data_json[i]['labels'], data_json[i]['video_id'], data_json[i]['video_path'], data_json[i]['type']]
            except:
    ...
    def __getitem__(self, index):
        if random.random() < self.mixup:
            ...
            label_indices = np.zeros(self.label_num) + (self.label_smooth / self.label_num)
            try:
                corruption_type = datum['type']
            except:
                corruption_type = 'weak'
            try:
        ...
        if not self.rt_idx:
            return fbank, image, corruption_type, label_indices
        else:
            return fbank, image, corruption_type, label_indices, index
