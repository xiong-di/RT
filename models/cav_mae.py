...
class Sine_Adapter(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, r=8, omega=100.0, merge=False):
        """
        r: 
        omega: Frequency of the sine function
        """
        ...
        if r > 0:
            ...
            self.w_down = nn.Parameter(torch.zeros(r, in_features))
    ...
    def forward(self, x):
        ...
        if self.r > 0:
            ...
            low_rank_matrix = self.w_up @ self.w_down

            # Applying sine activation and frequency modulation
            sine_activated_weight = torch.sin(self.omega * low_rank_matrix)
            
            # Apply Gain Control
            final_delta_weight = sine_activated_weight / self.g 

            sine_out = F.linear(x, final_delta_weight)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0., 
        ...
        #  Sine_Adapter
        self.qkv = Sine_Adapter(dim, dim * 3, bias=qkv_bias, r=r, omega=omega)
        
        if type == 'fuse':
            self.q = Sine_Adapter(dim, dim, bias=qkv_bias, r=r, omega=omega)
            self.k = Sine_Adapter(dim, dim, bias=qkv_bias, r=r, omega=omega)
            self.v = Sine_Adapter(dim, dim, bias=qkv_bias, r=r, omega=omega)
    ...
    def forward(self, x, ft=False):
        ...
        if N >= 512 + 196:
            if self.first_batch:
                ...
                if hasattr(self.q, 'w_down'):
                     self.q.w_down.data = self.qkv.w_down.data.clone()
                     self.q.w_up.data = self.qkv.w_up.data[:self.dim, :].clone()
                     self.q.g.data = self.qkv.g.data.clone() 

                     self.k.w_down.data = self.qkv.w_down.data.clone()
                     self.k.w_up.data = self.qkv.w_up.data[self.dim:self.dim*2, :].clone()
                     self.k.g.data = self.qkv.g.data.clone()

                     self.v.w_down.data = self.qkv.w_down.data.clone()
                     self.v.w_up.data = self.qkv.w_up.data[self.dim*2:, :].clone()
                     self.v.g.data = self.qkv.g.data.clone()
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, type='nofuse', r=4, omega=100.0): # <--- omega
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.norm1_a = norm_layer(dim)
        self.norm1_v = norm_layer(dim)
...
class CAVMAEFT(nn.Module):
    def __init__(self, label_dim, img_size=224, audio_length=1024, patch_size=16, in_chans=3,
        ...
        self.blocks_a = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, r=r, omega=omega) 
            for i in range(modality_specific_depth)])
        self.blocks_v = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, r=r, omega=omega) 
            for i in range(modality_specific_depth)])
        self.blocks_u = nn.ModuleList([
            Block(embed_dim, num_heads, mlp_ratio, qkv_bias=True, qk_scale=None, norm_layer=norm_layer, type='fuse', r=r, omega=omega) 
